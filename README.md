...
섹터 → 세부 산업 → 기업으로 내려가며 사업 구조와 실제 공시 숫자를 함께 읽는 개인 투자 공부용 아카이브입니다. **11개 섹터·58개 세부 산업·206개 기업 상세**를 수록합니다. Pretendard와 네이비·블루 팔레트를 사용하며, 결과물은 오프라인에서도 열리는 단일 HTML입니다.
...
트리맵 면적은 **수록 기업 수**를 나타냅니다. 시가총액이나 투자 비중이 아닙니다. 영업이익률 색상은 기업별 최근 확보 연간 수치이므로 회계기간과 업종 차이를 함께 확인하세요. 상위 분류는 GICS 체계를 참고하고 하위 산업은 자체 학습 분류를 사용합니다. 실시간 주가·수익률·예상 실적·매매 추천은 제공하지 않습니다.

### 수록 자료의 범위

아래는 **2026-09-20 검증 기준**입니다. 이후 갱신 결과는 HTML의 **업데이트 현황**에서 확인하세요.

| 항목 | 범위 |
|---|---|
| 산업 학습 자료 | 11개 섹터, 58개 산업, KPI 174개, 용어 36개 |
| 기업 상세 | 206개: 사업·매출 구조·연혁·강약점·경쟁사·관찰 질문·출처 |
| 연간 재무 / 별도 분기 재무 | 206개 / 187개 기업, 확보 가능한 최근 3개 연간 실적 표시 |
| 최신 분기 반영 지연 | 22개 기업, 실제 수치 기간과 최신 공시 원문을 화면에 표시 |
| 정량 매출 구성비 | 금액·분모를 확인한 7개 기업, 나머지는 수익원별 정성 설명 |

206개 기업에 재무 자료가 있다는 뜻이며 모든 지표와 최신 분기가 완비되었다는 뜻은 아닙니다. 기업마다 결산월·공시 주기·통화가 다릅니다. **설명의 검토일, 실적의 보고기간, 공시 제출일, 데이터 조회일은 서로 다른 날짜**입니다. 공식 IR·공시로 보완한 수치와 법인 승계·합병에 따른 범위 차이도 기업 화면에 표시합니다. 자세한 확인 결과는 [검증 기록](docs/VALIDATION.md)을 참고하세요.

## 사용 순서

1. 산업 지도에서 섹터를 선택하거나 한글·영문·티커로 검색합니다.
2. 세부 산업의 수익 구조와 KPI를 읽고 기업 타일을 선택합니다.
3. 기업의 수익원·연혁·경쟁 구도와 실제 재무 수치를 함께 확인합니다.
4. 기업 페이지의 기간 선택으로 최근 분기와 연간 실적을 비교하고 원문 링크를 엽니다.
...
`update.bat`은 기존 재무 파일과 HTML을 `backups/`에 보관합니다. 기업별 조회에 실패하면 확보된 기존 자료를 유지하고 `data/update-status.json` 및 `logs/`에 결과를 기록합니다. **사업·연혁·강약점·경쟁 구도는 공식 자료를 읽고 수동으로 갱신하는 콘텐츠**입니다. 배치는 서술이나 브라우저 개인 메모를 자동 수정하지 않으며, 설명 검토일 이후의 공시가 확인된 기업을 다시 검토할 대상으로 표시합니다.
...
| 기업별 사업·연혁·강약점·경쟁사·질문 | `data/companies/<TICKER>.json` |
